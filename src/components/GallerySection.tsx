...
function PhotoCard({ photoUrl, idx, style, photoDate }: PhotoCardProps) {
  ...
  const messages = [
    "เวลาอยู่กับแฟนเค้ามีความสุขมั๊กก",
    "ถ้าแฟนไม่รักเค้าหน้าแฟนจะเป็นเหมือนFilterแน่ หึๆ",
    "เหมือนเราถ่ายคู่กันเลยย",
    "อันนี้เป็นรูปคู่รูปแรกของเค้า เขินอ่ะ",
    "แฟนจุ๊บแก้มเค้าด้วยย",
    "อันนี้ตลกสุดๆเลย",
  ];

  // ฟังก์ชันช่วยจัดการตำแหน่งโฟกัสของรูปภาพแต่ละใบ
  const getImagePositionClass = (index: number) => {
    if (index === 2) return 'object-top';  // 🎯 รูปที่ 3 (index 2) เลื่อนภาพไปโฟกัสขยับขึ้นด้านบน
    return 'object-center';                // รูปอื่นๆ รวมถึงรูปที่ 1 (index 0) จะอยู่ตรงกลางปกติ ✨
  };
